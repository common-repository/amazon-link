...
'UnboxVideo', 'VHS', 'Video', 'VideoGames', 'Watches', 'Wireless', 'WirelessAccessories'),
            ...
            'au' => array('All', 'Baby', 'SportingGoods', 'Electronics', 'HomeImprovement', 'Kitchen', 'Books', 'Music', 'DVD', 'Watches', 'HealthPersonalCare', 'Software', 'VideoGames', 'KindleStore'));
...
/*****************************************************************************************/
      ...
      function grab_image ( $asin, $post_id = 0) {
         ...
         $this->in_post = false;
         $shortcode = array('args' => 'localise=0&asin='.$asin.'&template_content=%IMAGE%');
         //$data['asin']= array($settings['default_cc'] => $asin);
         //$data['template_content'] = '%IMAGE%';
         //$data['local_cc'] = $settings['default_cc'];
         //$image_url = $this->alink->make_links( $data );
         $image_url = $this->alink->shortcode_expand( $shortcode );
         ...
         foreach ($data as $item => $content) {
            if ( is_array($content) ) {
               $data[$item] = reset($content); //data[$item][0];
            } 
         }
