...
if ( ! class_exists ( 'Amazon_Link_Admin_Support' ) ) {
   class Amazon_Link_Admin_Support extends AmazonWishlist_For_WordPress {
      ...
      function amazon_admin_scripts() {
         ...
         /*
          * Run this to add Code Mirror
          */
          if ( 'amazon-link_page_amazon-link-templates' !== get_current_screen()->id ) {
              return;
          }
          ...
          wp_add_inline_script(
              'code-editor',
              sprintf( 'jQuery( function() { wp.codeEditor.initialize( "Content1", %s ); } );', wp_json_encode( $settings )
              ));
      }
      ...
      function register_tinymce_button( $button_array ) {
         global $current_screen;
         $type = $current_screen->post_type;
         if( is_admin() && ( $type == 'post' || $type == 'page' ) ) {
            array_push( $button_array, 'awlwp_button' );
         }
         return $button_array;
      }
      ...
      function register_tinymce_plugin( $plugin_array ) {
         ...
         $type = $current_screen->post_type;
         if( is_admin() && ( $type == 'post' || $type == 'page' ) ) {
            $plugin_array['awlwp_plugin'] = $this->URLRoot . '/tinymce-plugin.js';
         }
         return $plugin_array;
      }
      ...
      function get_option_list( $option_list = array() ) {
         ...
         if (!isset($this->option_list)) {
            ...
            $option_list = array(
               ...
               'default_template' => array( 'Name' => __('Default Template', 'amazon-link') , 'Description' => __('Default template to use for all links <em>* <a href="#aws_notes" title="AWS Access keys required for full functionality">AWS</a> *</em>', 'amazon-link'), 'Type' => 'selection', 'Class' => 'al_border'  ),
               ...
               'countries'     => array( 'Type' => 'checkboxes', 'Name' => __('Enabled Countries', 'amazon-link'), 'Class' => 'alternate al_border', 'Description' => __('Select which countries to support, disabled countries will use the default country above.', 'amazon-link')),
               ...
               'previews_off' => array( 'Name' => __('Disable Template Previews', 'amazon-link'), 'Description' => __('If the templates page does not load, try disabling all the Template Previews.', 'amazon-link'), 'Type' => 'checkbox', 'Size' => '40', 'Class' => 'alternate al_border' ),
               'debug'          => array( 'Name' => __('Debug Output', 'amazon-link'), 'Description' => __('Adds hidden debug output to the page source to aid debugging. <b>Do not enable on live sites</b>.', 'amazon-link'), 'Type' => 'checkbox', 'Size' => '40', 'Class' => 'al_border' ),
               'full_uninstall' => array( 'Name' => __('Purge on Uninstall', 'amazon-link'), 'Description' => __('On uninstalling the plugin remove all Settings, Templates, Associate Tracking IDs, Cache Data & ip2nation Data .<b>Use when removing the plugin for good</b>.', 'amazon-link'), 'Type' => 'checkbox', 'Size' => '40', 'Class' => 'alternate' ),
               'hd5e'           => array( 'Type' => 'end')
            );
...
/*****************************************************************************************/
      ...
      function saveOptions( $options ) {
         ...
         $options['plugin_extras'] = !empty($options['plugin_ids']);
