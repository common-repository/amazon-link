...
/*****************************************************************************************/
   ...
   $options['tg_head'] = array( 'Type' => 'section', 'Value' => __( '2. Choose which countries to support and enter your Amazon Affiliate IDs to earn commission on any Amazon sales. Valid Affiliate IDs can be obtained from the relevant Amazon sites: ', 'amazon-link'), 'Title_Class' => 'al_subhead2' );
   ...
   foreach ($country_data as $cc => $data) {
      ...
      $options['en_' . $cc] = array('Type' => 'checkbox', 'Default' => 0,
                                     'Name' => '<img style="height:14px;" src="'. $data['flag'] . '"> ' . $data['country_name'],
                                     'Hint' => sprintf(__('Select to enable %1$s.', 'amazon-link'), $data['country_name'] ), 'Input' => 'tag_'.$cc);
      $options['tag_' . $cc] = array('Type' => 'ignore', 'Default' => '', 'Class' => $pad,
                                     'Name' => '<img style="height:14px;" src="'. $data['flag'] . '"> ' . $data['country_name'],
...
/*****************************************************************************************/
   ...
   if(  $Action == __('Update Options', 'amazon-link') ) {
      ...
      foreach ($options as $opt => $opt_data) {
         if ( isset($opt_data['Name']) && empty($opt_data['Read_Only']) ) {
            ...
            if ( ($opt == 'localise') ) {
               ...
               if ( empty($opts[$opt]) || ($opts[$opt] != stripslashes($_POST[$opt]) )) {
                  $AWS_keys_updated = 1;
               }
      ...
      // Update Country Settings (Channels and Countries Enabled)
      $opts['countries'] = array();
      foreach ( $country_data as $cc => $data ) {
         $channels['default']['tag_' . $cc] = $_POST['tag_' . $cc];
         unset($opts['tag_'. $cc]);
         if (!empty($opts['en_' .$cc])) $opts['countries'][$cc] = 1;
      }
...
/*****************************************************************************************/
   ...
   foreach ($country_data as $cc => $data) {
      ...
      $opts['en_' .$cc] = !empty($opts['countries'][$cc]);
   }
