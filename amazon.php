...
/*
Copyright 2018-2019 Paul Stuttard (email : wordpress_amazonlink@ redtom.co.uk)
...
if (!class_exists('AmazonWishlist_For_WordPress')) {
   class AmazonWishlist_For_WordPress {
      ...
      function footer_scripts() {
         ...
         $settings       = $this->get_default_settings();
         $link_templates = $this->get_link_templates($settings);
         ...
         wp_localize_script( 'amazon-link-script', 
                             ...
                             array('link_templates' => $link_templates, 
                                   'country_data'   => $this->get_country_data( null, true ),
            ...
            $this->keywords = array(
               ...
               'manufacturer' => array( 'Live' => '1', 'Group' => 'ItemAttributes',
                                        'Position' => array(array('ItemAttributes','Manufacturer'),
                                                            array('ItemAttributes','Brand'))),
               'thumb'        => array( 'Live' => '1', 'Group' => 'Images', 'Default' => $this->URLRoot. '/'. 'images/no-img-lg-uk.gif',
                                        'Position' => array(array('MediumImage','URL'))),
               'image'        => array( 'Live' => '1', 'Group' => 'Images', 'Default' => $this->URLRoot. '/'. 'images/no-img-lg-uk.gif',
      ...
      function get_country_data( $cc = NULL, $filtered = false ) {
         ...
         if ( ! isset( $this->country_data ) ) {
            ...
            $this->country_data = apply_filters( 'amazon_link_get_country_data', array(
               ...
               'au' => array( 'cc' => 'au', 'mplace' => 'AU', 'mplace_id' => '36', 'lang' => 'en',     'flag' => $this->URLRoot. '/'. 'images/flag_au.png', 'tld' => 'com.au',    'language' => 'English',    'region' => 'na', 'imp' => 'ir-au', 'rcm' => 'rcm-au.amazon.au',             'site' => 'https://affiliate-program.amazon.com.au', 'buy_button' => 'https://images-na.ssl-images-amazon.com/images/G/15/buttons/buy-from-tan.gif', 'country_name' => 'Australia', 'link_close' => '</a>'),
            ), $this);
         }
         if ($filtered)
         {
            $settings = $this->get_default_settings();
            $country_data = array_intersect_key($this->country_data, ($settings['countries']));
         }
         else
         {
          ...
          * - Add line in map_country
          * - Add icon to images directory
          * - Update amazon-search.php to support Searches
          */
      }

      function get_link_templates($settings) {

         if ( ! isset( $this->link_templates ) ) {
            
            if (!empty($settings['aws_valid']))
            {
               $aws_params='&amp;SubscriptionId=%PUB_KEY%';
            }
            else
            {
               $aws_params='';
            ...
            $this->link_templates = apply_filters( 'amazon_link_multi_link_templates', 
                                                   array( 'A'=>'http://www.amazon.%TLD%/gp/product/%ARG%?ie=UTF8&amp;linkCode=xm2&amp;camp=2025&amp;creative=6738&amp;tag=%TAG%%CC%#&amp;creativeASIN=%ARG%'.$aws_params,
                                                          'S'=>'http://www.amazon.%TLD%/mn/search/?_encoding=UTF8&amp;linkCode=xm2&amp;camp=2025&amp;creative=19450&amp;tag=%TAG%%CC%#&amp;field-keywords=%ARG%'.$aws_params,
                                                          'R'=>'http://www.amazon.%TLD%/review/%ARG%?ie=UTF8&amp;linkCode=xm2&amp;camp=2025&amp;creative=6738&amp;tag=%TAG%%CC%#'.$aws_params,
                                                          'B'=>'http://www.amazon.%TLD%/e/e/%ARG%?tag=%TAG%%CC%#&amp;linkCode=xm2&amp;camp=2025'.$aws_params,
                                                          'U'=>'%ARG%',
                                                          'X'=>'%ARG%'),
                                                   $this);
         }
      ...
      function get_option_list( $option_list = array() ) {
         ...
         if ( ! isset( $this->option_list ) ) {
            ...
            // Populate the defaults - only aspects needed for Frontend
            ...
            $this->option_list['countries']['Default'] = array('uk' => 1, 'us' => 1, 'fr' => 1, 'de' => 1, 'it' => 1, 'es' => 1, 'in' => 1, 'mx' => 1, 'cn' => 1, 'jp' => 1, 'ca' => 1, 'br' => 1 );
      ...
      function get_channels( $override = False ) {
         ...
         if ( ! isset( $this->channels ) || ! $override ) {
            ...
            if ( ! empty($settings['plugin_ids']) ) {
                ...
                */
               $default_tags = array( 'tag_uk' => 'al-uk-21', 'tag_us' => 'al-us-20', 'tag_de' => 'al-de-21', 'tag_es' => 'al-es-21', 'tag_fr' => 'al-fr-21', 'tag_jp' => 'al-jp-22', 'tag_it' => 'al-it-21', 'tag_cn' => 'al-cn-23', 'tag_in' => 'al-in-21', 'tag_ca' => 'al-ca-20', 'tag_br' => 'al-br-20', 'tag_mx' => 'al-mx-20', 'tag_au' => 'al-au-22');
            } else {
               $default_tags = array( 'tag_uk' => '', 'tag_us' => '', 'tag_de' => '', 'tag_es' => '', 'tag_fr' => '', 'tag_jp' => '', 'tag_it' => '', 'tag_cn' => '', 'tag_in' => '', 'tag_ca' => '', 'tag_br' => '', 'tag_mx' => '', 'tag_au' => '');
            }
      ...
      function map_country( $cc ) {
         ...
         if ( ! empty( $country_map[$cc] ) ) {
            if (!empty($settings['countries'][$country_map[$cc]] )) {
      ...
      function get_country( $settings ) {
         ...
         if ( ! empty( $settings['localise'] ) ) {
            ...
            if ( empty( $this->local_country ) ) {
               ...
               } else if (isset($this->ip2n)) {
                  $cc = $this->ip2n->get_cc();
               } else {
                  return $settings['default_cc'];
               }
               $this->local_country = apply_filters( 'amazon_link_map_country', $cc, $this );
            }
      ...
      function get_local_info( $settings, $filtered=true) {

         return $this->get_country_data( $this->get_country( $settings, $filtered ) );
      ...
      function shortcode_expand ( $args ) {
         ...
         return apply_filters( 'amazon_link_shortcode_output', $output, $this, $settings );
      }
      ...
      function parse_shortcode( &$split ) {
         ...
         $settings = $this->get_country_data( null, false );
      ...
      function make_links( $settings )
      {
          ...
          */
         if ( !empty( $settings[$cc]['default_template'] ) && !isset( $settings[$cc]['template'] ) && !isset( $settings[$cc]['template_content'] ) ) {
            $settings[$cc]['template'] = $settings[$cc]['default_template'];
         }
         ...
         if ( $settings[$cc]['template_type'] == 'Multi' ) {
            ...
            $countries = array_keys( $this->get_country_data( null ) );
            $count = 1;
      ...
      function parse_template ( $item ) {
         ...
         $countries_a = array_keys( $this->get_country_data(null) );
      ...
      function parse_template_callback ( $args ) {
          ...
          */
         if ( !empty($settings['live']) && !empty($settings['prefetch']) && empty( $this->temp_data[$country]['prefetched']) && ! empty($asin) ) {
         ...
         if ( is_array( $phrase )) {
            $phrase = !empty($phrase[$keyword_index]) ? $phrase[$keyword_index] : ( !empty($phrase[0]) ? $phrase[0] : '');
         }
      ...
      */
      ...
      function get_url( $url, $type, $data, $search, $cc, $settings ) {
         ...
         $links = $this->get_link_templates($settings);
      ...
      function create_popup ( $data, $text ) {
         ...
         $sep = '';
         $term ='{';
         $countries = array_keys($this->get_country_data( null, true ));
      ...
      function doQuery( $request, $settings )
      {
         ...
         {
            return array('Error' => array ('Message' => 'AWS keys not set', 'Code' => 'NoKeys'));
         }
